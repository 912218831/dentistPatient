...
- (UIImage *)snapCurrentRgbData:(uint8_t *)rgb24 videoWidth:(uint32_t)videoWidth videoHeight:(uint32_t)videoHeight{
    
    int bitsPerComponent = 8;
    int bitsPerPixel = 24;
    int bytesPerRow = 3 * videoWidth;
    ...
    CGDataProviderRef provider = CGDataProviderCreateWithData(NULL, rgb24, videoWidth * videoHeight * 3, NULL);
    
    CGImageRef imageRef =  CGImageCreate(videoWidth, videoHeight,
                                         bitsPerComponent,
                                         bitsPerPixel,
                                         bytesPerRow,
                                         colorSpaceRef,
...
//                        [self doSnapShotWithSnapPath:_snapShotStruct.filePath rgbData:rgb24 width:videoWidth height:videoHeight flip:0 toAlbum:_snapShotStruct.bToAlbum Audio:_snapShotStruct.bNeedSnapAudio];
                    [self snapCurrentRgbData:rgb24 videoWidth:videoWidth videoHeight:videoHeight];
                        free(rgb24);
                        captureSuccess = YES;
                }
                memset(&_snapShotStruct, 0, sizeof(_snapShotStruct));
